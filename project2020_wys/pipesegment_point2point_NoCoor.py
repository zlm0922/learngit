...
# 单个excel文件
xlsPath = arcpy.GetParameterAsText(1)# r'D:\智能化管线项目\雨污水管线数据处理\原始文件\上交资料0730\总厂雨水最新补测数据\2-管段探查表.xlsx'.decode('utf-8')


xlspath_coordinate = arcpy.GetParameterAsText(2)# r"D:\智能化管线项目\雨污水管线数据处理\原始文件\上交资料0730\总厂雨水最新补测数据\3-中线控制点探查表.xlsx".decode('utf-8')
...
def getRowIndex(table, RowName, colfld):
    try:
        ...
        for i in range(table.nrows):
            ...
            # pass
            arcpy.AddMessage("请确认{0}输入项是否正确.".format(RowName))
...
def segment_created():
    ...
    for i in range(1, nrows):
        ...
        try:
            ...
            # print L2, B2, H2
            arcpy.AddMessage(str(L2) + "\t" + str(B2) + "\t" + str(H2))
            # print '\n'
            arcpy.AddMessage('\n')
            row = cur.newRow()
    ...
    # tt = nrows- 1- cal
    # arcpy.AddMessage("完成"+ os.path.basename(xlsPath) + "入库"+","+ "总计入库条数:" + str(tt) )
    arcpy.AddMessage("完成{0}入库，总计入库{1}条数据.".format(os.path.basename(xlsPath),nrows - 1-cal) )
