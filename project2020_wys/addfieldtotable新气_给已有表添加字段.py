...
def addfld():
    for j in range(nrows):
        ...
        try:
            arcpy.AddField_management(table.cell(j, 0).value,table.cell(j, 1).value,table.cell(j, 3).value.upper(),
                                  field_length = table.cell(j, 4).value,field_alias=table.cell(j, 2).value)
            print "为{}添加字段{}完成!".format(table.cell(j, 0).value,table.cell(j, 1).value)
...
for i in range(myworkbook.nsheets):
    ...
    # 适用于往已存在表上addfield
    if myworkbook.sheet_names()[i] == "需要添加的字段 (11)":
    ...
    # if i == 0:
    #     print myworkbook.sheet_names()[i]
    #     arcpy.CreateFeatureclass_management("SDE.Pipe_Integrity",table.cell(2,0).value,"POINT","",'ENABLED', 'ENABLED', template)
