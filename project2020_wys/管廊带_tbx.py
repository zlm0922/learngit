...
xlspath_coordinate = arcpy.GetParameterAsText(0) # r"D:\智能化管线项目\雨污水管线数据处理\原始文件\上交资料0730\总厂补加高架线路\4污水汽提净化水\7-最新管架(墩）探查表模板0704.xls".decode('utf-8')
xlspath_attribute = arcpy.GetParameterAsText(1)# r"D:\智能化管线项目\雨污水管线数据处理\原始文件\上交资料0730\总厂补加高架线路\4污水汽提净化水\6-管廊（带）探查表.xls".decode('utf-8')
arcpy.env.workspace = arcpy.GetParameterAsText(2) #r'D:\智能化管线项目\雨污水管线数据处理\提交成果\JLSHYWS0730_4污水汽.gdb'
...
def create_polyline_geometry(cur,ncols_attr,X,Y,Z,RowNameS,RowNameE,fldname):
    ...
    for i in range(getRowIndex(table_coor,RowNameS,"PipeRackName"), getRowIndex(table_coor,RowNameE,"PipeRackName")+1):
        ...
        # point.Z = table_coor.cell(i, getColumnIndex(table_coor,Z)).value
            ...
            arcpy.AddError(eee)
            arcpy.AddMessage("请检查非英文字段行是否删除，坐标字段名称是否正确")
    row = cur.newRow()
