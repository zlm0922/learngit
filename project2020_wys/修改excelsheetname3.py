...
def setCellformat(sheet, row, col):  # 设置单元格的数据
    "set value of one cell sheet"
    sht = xlbook.Worksheets(sheet)
    # print sht.Cells(0,0).value
    sht.Cells(row, col).Font.Size = 20  # 字体大小
...
if __name__ == '__main__':
    ...
    # 数据表excel所在文件path
    wks = r'D:\智能化管线项目\雨污水管线数据处理\雨污水采集模板\test'.decode('utf-8')
    # global xlbook
            ...
            for filename in filenames:
                ...
                if not os.path.basename(xlpath).startswith('~$'):
                    ...
                    for col in range(1, usedrange.Columns.Count + 1):
                        ...
                        print color4
                        value4 = getCell(1, 4, col).Value
                        # getCell(1, 4, col).Clear
                        getCell(1, 4, col).ClearFormats
                        getRow(1,4).Clear
                        getCell(1, 6, col).Value =value4
                        getCell(1, 6, col).Font.Size = 10
                        getCell(1, 6, col).Font.Name = 'Tahoma'
                        getCell(1, 6, col).Font.ColorIndex = color4
